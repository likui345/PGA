...
def build_sg(sg_edges_list_fn, paf_fn, chr_paths_fn):
    overlap_set = set()
    edges = []
    ctg_edges = {}
    with open(chr_paths_fn,"r")  as f :
        for l in f:
            l = l.strip().split()
            edge = re.split("~|\|",l[6])
...
def traverse_sg(ctg, sg):
    ...
    for key in ctg.keys():
        ...
        new_ctg[ctg_id] = ctg_id, s0, v0, t0, ctg_length, ctg_score, utg_path
...
def run(reads_fasta_fn, sg_edges_list_fn,paf_fn, chr_paths_fn):
    sg,edges = build_sg(sg_edges_list_fn, paf_fn, chr_paths_fn)


    ctg = {}
    with open(chr_paths_fn) as f:
        for l in f:
            l = l.strip().split()
...
def main(argv=sys.argv):
    description = 'Generate the chromosome and tiling paths, given the string graph.'
    ...
    parser.add_argument('--chr-paths-fn', type=str,
            help='Input. File containing chromosome paths.',required = True)
